...
const vercelProdHost = env("VERCEL_PROJECT_PRODUCTION_URL")?.replace(/^https?:\/\//, "");
const vercelOrigin = vercelHost ? `https://${vercelHost}` : undefined;
const vercelProdOrigin = vercelProdHost ? `https://${vercelProdHost}` : undefined;
const explicitBaseURL = env("BETTER_AUTH_URL") ?? vercelProdOrigin ?? vercelOrigin;
...
const baseURL = explicitBaseURL ?? {
  ...
  allowedHosts: [...previewAllowedHosts, "localhost", "127.0.0.1", "[::1]", ...(vercelHost ? [vercelHost] : []), ...(vercelProdHost ? [vercelProdHost] : [])],
...
const vercelTrusted = [vercelOrigin, vercelProdOrigin].filter(
  (origin): origin is string => Boolean(origin),
);
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...LOCAL_DEV_ORIGINS, ...vercelTrusted.filter((o) => o !== explicitBaseURL)]
  : [
...
const databaseUrl = normalizePostgresUrl(env("HFM_DATABASE_URL") ?? env("DATABASE_URL"));

...
function isCandidate(element: Element | null): element is HTMLElement {
  if (!(element instanceof HTMLElement)) return false;
  if (element.closest("[data-hfm-editor-ignore]")) return false;
  if (element.matches("script, style, html, body")) return false;
  return Boolean(element.closest("h1,h2,h3,h4,h5,h6,p,a,button,span,strong,em,li,img"));
}

function candidateFrom(target: EventTarget | null): HTMLElement | null {
  if (!(target instanceof Element)) return null;
  const candidate = target.closest<HTMLElement>("h1,h2,h3,h4,h5,h6,p,a,button,span,strong,em,li,img");
  return isCandidate(candidate) ? candidate : null;
}
...
function stableSelector(element: HTMLElement) {
  ...
  while (current && current.tagName.toLowerCase() !== "body") {
    ...
    const preferred = stableClasses.find((name) => /^hfm|^room|^public|^price|^feature|^site|^hero/.test(name));
    if (preferred) part += `.${cssEscape(preferred)}`;
    ...
    parts.unshift(part);
    if (preferred && parts.length >= 2) break;
    current = parent;
...
function describe(element: HTMLElement) {
  ...
  return `${tag.toUpperCase()} · ${text.slice(0, 56) || "untitled element"}`;
}
...
export function VisualSiteEditor() {
  ...
  function wireFrame() {
    ...
    editorStyle.setAttribute("data-hfm-editor-ignore", "true");
    editorStyle.textContent = `
      [data-hfm-editor-hover="true"] {
        outline: 2px solid #e7ad3d !important;
        outline-offset: 4px !important;
        cursor: pointer !important;
      }
      [data-hfm-editor-selected="true"] {
        outline: 3px solid #40b9a4 !important;
    ...
    const clearHover = () => {
      if (hovered) hovered.removeAttribute("data-hfm-editor-hover");
      hovered = null;
    };
  ...
  function updatePreview(next: Selection) {
    setSelection(next);
  ...
  async function saveSelection() {
    ...
    try {
      ...
      void adminGetVisualOverrides({ data: { page } }).then((overrides) => setSavedCount(Object.keys(overrides).length));
    ...
    <div className="mt-7 overflow-hidden rounded-[28px] border border-white/10 bg-[#07110e] shadow-[0_28px_90px_rgba(0,0,0,.28)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-white/[.035] px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div>
            <p className="text-sm font-medium text-[#fff8ed]">Visual website editor</p>
            <p className="text-[11px] text-white/45">{savedCount} saved visual edit{savedCount === 1 ? "" : "s"} on this page</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="rounded-full bg-white/7 p-1">
            <button
              type="button"
              className={cn("rounded-full px-3 py-1.5 text-xs", viewport === "desktop" ? "bg-[#fff8ed] text-[#10281f]" : "text-white/65")}
            ...
            <button
              type="button"
              className={cn("rounded-full px-3 py-1.5 text-xs", viewport === "mobile" ? "bg-[#fff8ed] text-[#10281f]" : "text-white/65")}
          ...
          <button
            type="button"
            className="grid size-9 place-items-center rounded-full bg-white/7 text-white/70 hover:bg-white/12 hover:text-white"
      ...
      <div className="grid min-h-[760px] xl:grid-cols-[220px_minmax(0,1fr)_330px]">
            ...
            {PAGES.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  page === item.path ? "bg-[#2f8478] text-white" : "text-white/62 hover:bg-white/7 hover:text-white",
                )}
                onClick={() => {
                  cleanupRef.current?.();
                  setSelection(null);
        ...
        <aside className="border-t border-white/10 bg-[#0b1713] p-5 xl:border-l xl:border-t-0">
          {!selection ? (
            <div className="sticky top-4">
              <div className="grid size-11 place-items-center rounded-2xl bg-[#d3a34d]/12 text-[#d3a34d]">
              ...
              <p className="mt-3 text-sm leading-relaxed text-white/52">
                Hover the real page. Gold means editable. Click text, a photo, or a link and its controls open here.
              </p>
              <div className="mt-5 space-y-2 text-xs text-white/45">
                <p className="flex items-center gap-2"><Type className="size-3.5" /> Headings, paragraphs, buttons, labels</p>
            ...
            <div className="sticky top-4">
              ...
              {selection.tag === "a" ? (
                <div className="mt-5 flex gap-2">
                  <button
                    type="button"
                    className={cn("rounded-full px-3 py-1.5 text-xs", selection.kind === "text" ? "bg-[#fff8ed] text-[#10281f]" : "bg-white/7 text-white/60")}
                    onClick={() => updatePreview({ ...selection, kind: "text" })}
                  >
                    Edit label
                  </button>
                  <button
                    type="button"
                    className={cn("rounded-full px-3 py-1.5 text-xs", selection.kind === "link" ? "bg-[#fff8ed] text-[#10281f]" : "bg-white/7 text-white/60")}
                    onClick={() => {
                      const element = frameRef.current?.contentDocument?.querySelector(selection.selector);
                      const href = element instanceof HTMLAnchorElement ? element.getAttribute("href") ?? "/" : "/";
                      updatePreview({ ...selection, kind: "link", value: href });
                    }}
                  >
                    Edit link
                  </button>
              ...
              <div className="mt-5">
                <Label className="text-[#fff8ed]">
                  {selection.kind === "image" ? "Image URL" : selection.kind === "link" ? "Destination" : "Text"}
                </Label>
                {selection.kind === "text" && selection.value.length > 90 ? (
                  <Textarea
                    className="mt-2 min-h-40 bg-white/7 text-[#fff8ed]"
                    value={selection.value}
                    onChange={(event) => updatePreview({ ...selection, value: event.target.value })}
                  />
                ) : (
                  <Input
                    className="mt-2 bg-white/7 text-[#fff8ed]"
                    value={selection.value}
                    onChange={(event) => updatePreview({ ...selection, value: event.target.value })}
                  />
                )}
              ...
              {selection.kind === "image" ? (
                <div className="mt-4">
                  ...
                  <Textarea
                    className="mt-2 min-h-24 bg-white/7 text-[#fff8ed]"
                    value={selection.alt}
                    onChange={(event) => updatePreview({ ...selection, alt: event.target.value })}
                  />
                  <p className="mt-2 text-xs leading-relaxed text-white/38">Keep this literal and descriptive for screen readers.</p>
                </div>
              ) : null}

              <div className="mt-6 flex flex-wrap gap-2">
                <Button type="button" variant="gold" disabled={busy || !selection.value.trim()} onClick={() => void saveSelection()}>
                ...
                <Button
                  ...
                  variant="outline"
                  disabled={busy}
                  className="border-white/20 bg-transparent text-[#fff8ed]"
              ...
              <p className="mt-4 text-xs leading-relaxed text-white/38">
                Changes are stored as safe text, image, or link overrides—never executable HTML.
              </p>
            </div>
          )}
        </aside>
